...
from core import is_iterable, dttms, BDOffset
...
def get_destination(name):
    return settings.report_destinations.get(name, settings.report_destinations.get("default", settings.email_address))


def send_report(name, content, dttm=..., subject=...):
    if dttm is ...:
        dttm = dttms.format_dt_sql(dttms.today() - BDOffset(1))
    if subject is ...:
        subject = f"[Report {dttm}] {name}"
    to = get_destination(name)
...
def send_email(
    ...
    try:
        ...
        if to_emails is ...:
            to_emails = settings.email_address
