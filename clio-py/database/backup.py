...
from core import services, DttmLike
from core.collections import compare_collections
...
def compare_dbs():
    ...
    db_backup = services.audit_db_backup()
...
def main():
    ...
    with ExitStack() as exit_stack:
        if args.compare:
            exit_stack.enter_context(services.audit_db().stop_merges())
            exit_stack.enter_context(services.audit_db_backup().stop_merges())
        ...
        if args.restore_primary:
            ...
            if args.compare:
                exit_stack.enter_context(services.audit_db().stop_merges())  # restore removes stop merges command
