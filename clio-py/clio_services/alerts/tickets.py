...
from clio import DttmLike, dbs, dttms, strs, Keyed, Key, arrays
import clio_services
from clio_services.alerts import Ticket
from clio.collections import ensure_list
from clio.strs import from_jsonl


def _read_checks():
    result = {}

    if clio_services._tasks_dfn is None:
        return result

    for task in from_jsonl(clio_services._tasks_dfn):
        task_dfn = task["task"]
...
class TicketsWidget:
    ...
    def create(self, id: str, message: str, sos: bool = False):
        ...
        clio_services.audit_db().insert(ticket)
    ...
    def ack(self, ticket: str, user: str = None, _username=None):
        ...
        clio_services.audit_db().insert(update)
...
class ScheduleAllocator:
    ...
    def get_prefs(self, usernames=..., as_of_dttm=..., parsed=True):
        ...
        prefs = clio_services.audit_db().select(SchedulePreference, query)
        if parsed:
